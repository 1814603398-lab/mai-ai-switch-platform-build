...
This directory contains the Flatpak manifest (`com.ccswitch.desktop`) for MaiAI Switch, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo MaiAI-Switch-Linux.flatpak com.ccswitch.desktop
```

4) Install and run:

```bash
flatpak install --user ./MaiAI-Switch-Linux.flatpak
